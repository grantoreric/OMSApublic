...
class HW2_sql():
    ...
    def part_d(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_d_sql = """
            SELECT cast_name, COUNT (movie_id)
            FROM movie_cast
            WHERE popularity >10
            GROUP BY cast_name
    ...
    def part_e(self,connection):
        ...
        part_e_sql = """
            SELECT 
                ...
                printf('%.2f', CAST(movies.score AS FLOAT)) as movie_score,
            ...
            ORDER BY 
                CAST(movies.score AS FLOAT) DESC,
                COUNT(cast_id),
                movies.title
            LIMIT 5
    ...
    def part_f(self,connection):
        ...
        SELECT
            cast_id,
            cast_name,
            printf('%.2f',avg(CAST(score as FLOAT))) as average_score
        FROM movie_cast
        INNER JOIN movies
    ...
    def part_g(self,connection):
        ...
        part_g_sql = """
            ...
            SELECT 
                c.cast_id1 as cast_member_id1,
                c.cast_id2 as cast_member_id2,
                COUNT(movies.id) as movie_count,
                printf('%.2f', avg(CAST(score as FLOAT))) as average_movie_score
            FROM ((
                SELECT 
                    cast_id as cast_id1,
                    movie_id
            ...
            WHERE c.cast_id1 > c.cast_id2
            GROUP BY
                c.cast_id1,
                c.cast_id2
            HAVING COUNT(c.movie_id) > 2 AND avg(CAST(movies.score as FLOAT)) >=40
    ...
    def part_gi(self,connection):
        ...
        part_g_i_sql = """
            SELECT 
                d.cast_id as cast_id,
                cast_bio.cast_name as cast_name,
                printf('%.2f',avg(CAST(d.collaboration_score as FLOAT))) as collaboration_score
            FROM
                (SELECT
